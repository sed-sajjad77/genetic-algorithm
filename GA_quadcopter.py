...
def perform_action(env, pos, gene):
    ...
    if action == 1: new_pos[1] += 1
        ...
        new_pos = [pos[0] + np.random.choice([-1, 1]), pos[1] + np.random.choice([-1, 1])]
...
def select_parents(population, scores, number):
    ...
    # if scores.sum() == 0:
    #     print(population)
    parents_indices = np.random.choice(np.arange(len(population)), size=number, replace=False, p=scores/scores.sum())
...
def main():
    ...
    best_gene, best_score, change_generations, best_scores = genetic_algorithm(env ,population, population_size)
    print("Best Gene:", best_gene)
    print("Best Score:", best_score)

    plt.figure(figsize=(10, 6))
    plt.plot(best_scores, marker='o', linestyle='-', color='b', label='Best Score')

    for gen in change_generations:
        plt.scatter(gen, best_scores[gen], color='red', zorder=5, label='Improvement' if gen == change_generations[0] else "")
